...
public:
    virtual string getName() = 0;
    virtual int getSize() = 0;
    virtual string openAll(int tabSpace=0) = 0;
    virtual string ls() = 0;
...
class FileSystem {
public:
    void getStructure(FileSystemComponent* folder) {
        cout << folder->openAll() << endl;
    }

    void ls(FileSystemComponent* folder) {
        cout << folder->ls() << endl;
    }
};
...
public:
    ...
    string openAll(int tabSpace=0) {
        ...
        for (auto child : childrens) {
            result += child->openAll(tabSpace+1) + "\n";
        }

        return result;
    }

    void addChildren(FileSystemComponent* child) {
...
int main() {
    ...
    root->addChildren(new File("a.txt"));
    root->addChildren(new File("b.txt"));

    Folder* documents = new Folder("documents");
    documents->addChildren(new File("resume.pdf"));

    root->addChildren(documents);

    Folder* childDocs = new Folder("childDocs");

    documents->addChildren(childDocs);

    fSystem->getStructure(root);
